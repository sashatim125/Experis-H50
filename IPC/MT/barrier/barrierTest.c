...
static void* Summarizer(void* _p)
{
	...
	unsigned long i, j, correctSum, problemRounds;
	...
	{	
		...
		if(0 != Barrier_Wait(params->m_barrier))
		{
			fprintf(stderr,"Summarizer: Barrier_Wait on enter error!\n");
			return (void*)(-1);
		}
		params->m_sum = 0;
		...
		if(0 != Barrier_Wait(params->m_barrier))
		{
			fprintf(stderr,"Summarizer: Barrier_Wait on exit error!\n");
			return (void*)(-2);
		}
...
static void* Counter(void* _p)
{
	...
	for(j=0 ; j < params->m_numOfRounds ; ++j)
	{
		...
		if(0 != Barrier_Wait(params->m_barrier))
		{
			fprintf(stderr,"Counter: Barrier_Wait on summarizer start error!\n");
			return (void*)(-1);
		}
	
		if(0 != Barrier_Wait(params->m_barrier))
		{
			fprintf(stderr,"Summarizer: Barrier_Wait on summarizer finish error!\n");
			return (void*)(-2);
		}
	}
