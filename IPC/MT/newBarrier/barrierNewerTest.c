...
#define DEFAULT_MULT_FACTOR 2;

#define USAGE_NOTE do{\
	fprintf(stderr, "Usage: %s [-c] num_of_counters [-r] num_of_rounds [-m] mult_factor\n",_argv[0]);\
}while(0)
...
static int parseArgs(int _argc, char *_argv[], Args *_args)
{
    ...
    while ((opt = getopt(_argc, _argv, "c:r:m:")) != -1)
    {
    	switch(opt)
    	{
    		case 'r':
    		...
    		case 'c':
    			...
    			break;
    		case 'm':
	    		sscanf(optarg,"%lu",&(_args->m_multFactor));
    			break;
    ...
    if(0 == _args->m_multFactor)
    {
    	_args->m_counters = DEFAULT_MULT_FACTOR;
    }
...
int main(int _argc, char *_argv[])
{
	...
	arguments.m_counters *= arguments.m_multFactor;
	...
	if(NULL == (parStruct.m_counters = calloc(arguments.m_counters,sizeof(*(parStruct.m_counters)))))
	{
	...
	parStruct.m_numOfRounds = arguments.m_rounds*arguments.m_multFactor;
