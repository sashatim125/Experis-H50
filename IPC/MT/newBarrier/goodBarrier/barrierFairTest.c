...
#define DEFAULT_GROUPS 1

#define USAGE_NOTE do{\
	fprintf(stderr, "Usage: %s [-c] num_of_counters [-r] num_of_rounds [-b] num_of_groups\n",_argv[0]);\
}while(0)
...
static int parseArgs(int _argc, char *_argv[], Args *_args)
{
    ...
    while ((opt = getopt(_argc, _argv, "c:r:g:")) != -1)
    {
    	switch(opt)
    	{
    		case 'r':
    		...
    		case 'c':
    			...
    			break;
    		case 'g':
	    		sscanf(optarg,"%lu",&(_args->m_groups));
    			break;
    ...
    if(0 == _args->m_groups)
    {
    	_args->m_counters = DEFAULT_GROUPS;
    }
...
static void* Summarizer(void* _p)
{
	...
	problemRounds = 0;
	for(j=0 ; j < params->m_numOfRounds*params->m_numOfGroups ; ++j)
	{	
...
static void* Counter(void* _p)
{
	...
	unsigned long i,j,idInGroup;
	
	sem_post(&(params->m_idSafeSem));
	
	idInGroup = myId % params->m_numOfGroups;
	
	for(j=0 ; j < params->m_numOfRounds ; ++j)
	{
		params->m_counters[idInGroup] = 0;

		for(i=1 ; i <= idInGroup ; ++i)
		{
			params->m_counters[idInGroup] += 1;
			usleep(100*(unsigned)i);
		}
...
int main(int _argc, char *_argv[])
{
	...
	parStruct.m_numOfGroups = arguments.m_groups;
	...
	if(NULL == (counterThreads = malloc(sizeof(pthread_t) * arguments.m_counters*arguments.m_groups)))
	{
	...
	for(j=0 ; j < arguments.m_counters*arguments.m_groups ; ++j)
	{	
		parStruct.m_myId = j;
