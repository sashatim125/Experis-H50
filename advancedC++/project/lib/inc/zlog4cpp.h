...
//	LOG_SEVERE		< Severe error requiring immediate intervention	
...
//DO NOT call directly , can result in wrong log record
	...
	//return 0 if such log does not exist
	static Zlog* GetLog(const std::string& _name);
...
private:
	...
	static std::map<std::string, Zlog*> s_logs;
	
	static std::string GetTimeStamp();
	
	static char GetLevelChar(LogLevels _logLevel);	
};
