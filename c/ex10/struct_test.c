...
void fprint_persons(FILE* file,person_t* persons, size_t size) {

	size_t i;
	
	for (i=0 ; i<size ; ++i){

		fprintf(file,"The %lu-th person's name is %s.\n",i+1,(persons+i)->name);
		fprintf(file,"The %lu-th person's ID is %d.\n",i+1,(persons+i)->id);
	}
}
...
int main (){
	...
	person_t persons[PERSONS] , persons2[PERSONS];
	int i;
	FILE *f;
	char buffer[50];
...
*/
	...
	fprint_persons(f,persons,PERSONS);
